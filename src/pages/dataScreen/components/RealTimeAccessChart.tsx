...
import styles from "./RealTimeAccessChart.module.scss";
...
const RealTimeAccessChart = () => {
  ...
  return (
    <>
      <div className={styles["actual-total"]}>
        <div className={styles["expect-total"]}>
          可预约总量<i>999999</i>人
        </div>
        <div className={styles["actual-total"]}>
          {actualTotal.split("").map((item, index) => {
            return (
              <div className={styles["actual-item"]} key={index}>
                {item}
              </div>
            );
          })}
          <div className={styles["actual-item"]}>人</div>
        </div>
      </div>
      <div ref={echartsRef} className={styles["actual-echarts"]}></div>
    </>
  );
};
